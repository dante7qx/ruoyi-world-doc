...
本框架基于精灵精灵开发框架，集成工作流引擎 [Flowable](https://tkjohn.github.io/flowable-userguide/) 6.7.2。框架包含工作流涉及的组件，需要按文档集成到你现有的项目中，适用于进行过程中，需要集成工作流的项目。
...
解压下载后的文件。（可以将project-flowable 修改为 [项目]-flowable）

- 将`project-flowable`加入项目工程，并修改`pom.xml`
...
- 删除流程：每个业务根据流程类型实现相关删除
  ...
  代码在`com.spirit.flowable.service.impl.SysFlowTaskServiceImpl`下
  ``` java
  /**
	 * 删除流程 
